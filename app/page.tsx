const mockAffiliate = {
  name: "Spencer Smith",
  email: "spencer@easybuttonskiptrace.com",
  affiliateCode: "EBS-SPENCER",
  referralLink: "https://www.easybuttonsoftware.com/?ref=EBS-SPENCER",
};
...
function runTests() {
  const summary = getAffiliateSummary(productBreakdown, referrals);

  if (Math.round(summary.totalRevenue * 100) !== 1243055) {
    throw new Error("Total revenue calculation failed");
  }

  if (Math.round(summary.totalCommission * 100) !== 372917) {
    throw new Error("Total commission calculation failed");
  }

  if (Math.round(summary.pendingPayouts * 100) !== 8970) {
    throw new Error("Pending payout calculation failed");
  }

  if (Math.round(summary.paidOut * 100) !== 7500) {
    throw new Error("Paid payout calculation failed");
  }
}
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            Track commissions across Skip Tracing and CRM in one place.
          </h1>
        ...
        <div style={panelCard}>
          ...
          <p style={{ color: "#94a3b8", fontSize: 14 }}>
            Logged in as {mockAffiliate.email}
          </p>

          <div style={codeBox}>{mockAffiliate.affiliateCode}</div>
          <div style={linkBox}>{mockAffiliate.referralLink}</div>

          <button type="button" style={copyButton}>Copy Referral Link</button>

          <p style={{ color: "#64748b", fontSize: 13, marginTop: 18 }}>
            Front-end only version. Later, this can use Clerk for login,
            Supabase for affiliate rows, and FirstPromoter for live payouts.
          </p>
        </div>
      </section>
      ...
      <section id="referrals" style={{ padding: "0 40px 60px" }}>
        <div style={panelCard}>
          ...
          >
            ...
            <span style={pill}>Frontend ready</span>
          </div>
